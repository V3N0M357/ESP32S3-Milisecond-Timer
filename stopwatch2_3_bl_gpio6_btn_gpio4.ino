...
#define BTN_RESET        4  // GPIO4: free pin on 20-pin header (GPIO17 was R4 display pin)
#define HOLD_MS         2000
...
// ============================================================
// ============================================================
//  LGFX config for Waveshare ESP32-S3-LCD-Driver-Board + 1024x600 RGB panel
//  Pin mapping: confirmed from Waveshare 40-pin connector schematic
//  Bus_RGB pin order: d0-d4 = B[0:4], d5-d10 = G[0:5], d11-d15 = R[0:4]
// ============================================================
...
public:
  ...
  LGFX(void) {
    ...
    // --- CRITICAL: tell LovyanGFX to allocate framebuffer from PSRAM ---
    // Without this, 1024x600x2=1.2MB malloc fails on 288KB internal heap
    // → returns NULL → memset crash at EXCVADDR 0x00000000
    {
      auto cfg = _panel_instance.config_detail();
      cfg.use_psram = 1;
      _panel_instance.config_detail(cfg);
    }
    // --- RGB bus: Waveshare ESP32-S3-LCD-Driver-Board 40-pin pinout ---
    {
      auto cfg = _bus_instance.config();
      cfg.panel = &_panel_instance;
      // ── CONFIRMED pin map for Waveshare ESP32-S3-LCD-Driver-Board ──
      // Blue[0:4]
      cfg.pin_d0  =  5;  // B0
      cfg.pin_d1  = 45;  // B1
      cfg.pin_d2  = 48;  // B2
      ...
      cfg.pin_d13 =  8;  // R2
      cfg.pin_d14 = 18;  // R3
      cfg.pin_d15 = 17;  // R4  ← was our BTN_RESET pin! moved button to GPIO16
      // Sync/clock
      cfg.pin_henable = 40;  // DE
      cfg.pin_vsync   = 39;  // VSYNC
      cfg.pin_hsync   = 38;  // HSYNC
      cfg.pin_pclk    = 41;  // PCLK
      // 1024x600 timing
...
// ============================================================
//  Startup task — runs in its own FreeRTOS task with 64KB stack
// ============================================================
static void startupTask(void*) {
  // lcd->init() runs here in 64KB stack — avoids stack overflow on default loop task
  // Drive backlight HIGH before init so screen isn't dark
  // Waveshare ESP32-S3-LCD-Driver-Board: BL pin is GPIO6
  #define LCD_BL 6
  ...
  lcd->setRotation(0);
  lcd->fillScreen(TFT_BLACK);
  Serial.printf("Free heap after lcd init: %u bytes\n", (unsigned)ESP.getFreeHeap());
...
while (!Serial && millis() - t < 5000);  // wait up to 5s for serial
Serial.println("=== BOOTING ===");
  ...
  // All heavy init (lcd->init, startup animation) runs in 64KB task
  TaskHandle_t h;
  xTaskCreate(startupTask, "startup", 65536, NULL, 1, &h);
  while (eTaskGetState(h) != eDeleted) delay(10);
}
